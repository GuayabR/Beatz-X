...
const PLAYBACK_SPEED_MAX: float = 8.0
...
## Canvas item blend modes. (The ones under the visibility in the inspector will not work; Use these)
@export_enum("Mix (Default)", "Add", "Subtract", "Multiply", "Premultiplied Alpha") var blend_mode: int = 0: set = set_blend_mode

## Makes it so the darker a pixel is in your video, the more transparent that pixel is
## (Preferrably use this with the "Add" or "Premultiplied Alpha" blend mode for a brigher image)
@export var brightness_alpha_blend: bool = false: set = set_brightness_alpha_blend
...
func _enter_tree() -> void:
	...
	_shader_material.shader = _blend_shaders[0] # default to mix
...
var _blend_shaders := [
	preload("res://addons/gde_gozen/yuv_to_rgb.gdshader"),
	preload("res://addons/gde_gozen/yuv_to_rgb_add.gdshader"),
	preload("res://addons/gde_gozen/yuv_to_rgb_sub.gdshader"),
	preload("res://addons/gde_gozen/yuv_to_rgb_mul.gdshader"),
	preload("res://addons/gde_gozen/yuv_to_rgb_sub_premul_alpha.gdshader")
]

func set_blend_mode(value: int) -> void:
	blend_mode = value
	if _shader_material:
		_shader_material.shader = _blend_shaders[blend_mode]

func set_brightness_alpha_blend(value: bool) -> void:
	brightness_alpha_blend = value
	if _shader_material:
		_shader_material.set_shader_parameter("brightness_alpha_blend", value)
...
func _update_video(new_video: GoZenVideo) -> void:
	...
	_shader_material.set_shader_parameter("brightness_alpha_blend", brightness_alpha_blend)
...
func _process(delta: float) -> void:
	video_texture.self_modulate = self_modulate
