...
func _ready():
	...
	if selected_artist: load_artist()
...
func _process(_delta) -> void:
	...
	for item in add_queue:
		...
		var idx = list.add_item(entry["text"], preload("res://Resources/misc/noCover.png"))
...
func load_artist():
	print("Loading artist's charts")
	$top_intro/artist_name.text = selected_artist
...
#
	...
	set_process(true)
	_load_songs(selected_artist)
	
	loaded_charts.emit()

@onready var list = $song_list
...
func _load_songs(artist: String):
	list.clear()
	add_queue.clear()
	processing_index = 0
	grouped_songs = {}

	print("Starting song scan", " for artist: " + artist)
					...
					if FileAccess.file_exists(info_path):
						...
						if file:
							...
							if parsed and parsed.has("info"):
								var artist_name := str(parsed["info"].get("artist", ""))
...
func _on_song_list_gui_input(event: InputEvent) -> void:
	if event is InputEventMouseButton and event.pressed and Settings.misc.smooth_scrolls:
		if event.button_index == MOUSE_BUTTON_WHEEL_UP or event.button_index == MOUSE_BUTTON_WHEEL_DOWN:
			var scroll_bar = $song_list.get_v_scroll_bar()
			if not scroll_bar:
				return
